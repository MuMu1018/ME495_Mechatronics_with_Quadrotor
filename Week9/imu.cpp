...
#define A 0.004 // complementary filter constant
#define NEUTRAL 1200
#define PWM_MAX 1700
#define PWM_MIN 1000
#define KP 4.5				// 0 to 15	1.8
#define KD 320				// 10 to 100	100
#define KI 0.01				// less than 0.05
#define KP_yaw 4.5			// 0 to 15	1.8
#define KI_yaw 0.02				// less than 0.05
...
void set_PWM(int pwm, uint8_t channel, float time_on_us)
{
    ...
    if ((int)time_on_us > PWM_MAX)
        time_on_us = (float)PWM_MAX;
    if ((int)time_on_us < PWM_MIN)
        time_on_us = (float)PWM_MIN;
    ...
    else
    {
        ...
        while(execute==1)
        {
            ...
            if(s.sequence_num!=frame){
                ...
                desired_roll = s.roll;
                //printf("Desires roll: %f\n\r",desired_roll);
                desired_pitch = s.pitch;
                //printf("Desires pitch: %f\n\r",desired_pitch);
                desired_yaw = s.yaw;
                //printf("Desires yaw: %f\n\r",desired_yaw);
                neutral_power = s.thrust;
                ...
                else if(s.keypress==33){
                    pause = 1;
                }
                else if(s.keypress==34){
                    pause = 0;
                    neutral_power = NEUTRAL;
                }
                else if(s.keypress==35){
                    pause = 1;
                    calibration();
            ...
            */
            ...
            if(execute==1)//if cntrl+c has not been pressed
            {
                ...
                if ((roll_filtered > 40) || (roll_filtered < -40)){
                    
                    printf("Roll out!\n\r");
                    execute = 0;
                }
                if( (pitch_filtered > 40) || (pitch_filtered < -40) ){
                    printf("Pitch out!\n\r");
                    execute = 0;
                }
                if( (z_angle_dps > 120) || (z_angle_dps < -120) ){
                    printf("Yaw out!\n\r");
                    execute = 0;
                }
                if (diff > 5E8){
                    ...
                    P_term_yaw = error_yaw*KP_yaw;
                    I_term_yaw = I_term_yaw + error_yaw*KI_yaw;
                  
                    //printf("current_roll: %f	current_pitch: %f\r\n",current_roll,current_pitch);
                    
                    
                    //printf("desired_yaw = %f    current_yaw = %f\r\n",desired_yaw,current_yaw);
                    ...
                    /*int prop_PWM0 = neutral_power + 30 + 1.15 * (+ P_term_roll + KD*roll_velocity  - P_term_yaw )+ I_term_roll - I_term_yaw;
                    int prop_PWM1 = neutral_power - 30 + 0.8 * (- P_term_roll - KD*roll_velocity  -  P_term_yaw)- I_term_roll - I_term_yaw;
                    int prop_PWM2 = neutral_power + 50 + 2.5 * (- P_term_pitch - KD*pitch_velocity  + P_term_yaw)- I_term_pitch+ I_term_yaw;
                    int prop_PWM3 = neutral_power - 80 + 0.6 * (+ P_term_pitch + KD*pitch_velocity  + P_term_yaw )+ I_term_pitch+ I_term_yaw;
                    */
                    int prop_PWM0 = neutral_power + 30 + 1.15 * (+ P_term_roll + KD*roll_velocity  - P_term_yaw )+ I_term_roll - I_term_yaw;
                    int prop_PWM1 = neutral_power - 30 + 0.6 * (- P_term_roll - KD*roll_velocity  -  P_term_yaw)- I_term_roll - I_term_yaw;
                    int prop_PWM2 = neutral_power + 50 + 2.5 * (- P_term_pitch - KD*pitch_velocity  + P_term_yaw)- I_term_pitch+ I_term_yaw;
                    int prop_PWM3 = neutral_power - 80 + 0.5 * (+ P_term_pitch + KD*pitch_velocity  + P_term_yaw )+ I_term_pitch+ I_term_yaw;
                    //printf("z_angle_dps: %f\r\n",z_angle_dps);

                    //int prop_PWM0 = neutral_power + 5  + P_term_yaw + I_term_yaw;
                    //int prop_PWM1 = neutral_power + P_term_yaw + I_term_yaw;
                   ...
                   /* int prop_PWM0 = 1100;
                    ...
                    int prop_PWM3 = 1100;
                    */
                    //int prop_PWM2 = neutral_power  - 125 - P_term_yaw - I_term_yaw;
                    //int prop_PWM3 = neutral_power  - P_term_yaw - I_term_yaw;
                    ...
                    // limits on motor duty cycle
                    if (prop_PWM0 > PWM_MAX)
                        prop_PWM0 = PWM_MAX;
                    if (prop_PWM0 < PWM_MIN)
                        prop_PWM0 = PWM_MIN;
                    if (prop_PWM1 > PWM_MAX)
                        prop_PWM1 = PWM_MAX;
                    if (prop_PWM1 < PWM_MIN)
                        prop_PWM1 = PWM_MIN;
                    if (prop_PWM2 > PWM_MAX)
                        prop_PWM2 = PWM_MAX;
                    if (prop_PWM2 < PWM_MIN)
                        prop_PWM2 = PWM_MIN;
                    if (prop_PWM3 > PWM_MAX)
                        prop_PWM3 = PWM_MAX;
                    if (prop_PWM3 < PWM_MIN)
                        prop_PWM3 = PWM_MIN;
                    
                    // set duty cycle
                    //printf("motor 0: %d	motor 1: %d motor 2: %d	motor 3: %d\r\n",prop_PWM0,prop_PWM1,prop_PWM2, prop_PWM3);
                    //printf("error_yaw: %f  motor 0: %d	motor 1: %d motor 2: %d	motor 3: %d\r\n",error_yaw,prop_PWM0,prop_PWM1,prop_PWM2, prop_PWM3);
